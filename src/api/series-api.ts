...
import { SeriesTPOProfile } from '../model/series-tpos';
import {
	SeriesOptionsMap,
...
export class SeriesApi<TSeriesType extends SeriesType> implements ISeriesApi<TSeriesType> {
	...
	public setTPOs(data: SeriesTPOProfile<Time>[]): void {
		checkItemsAreOrdered(data, true);

		const convertedTPOs = data.map<SeriesTPOProfile<TimePoint>>((tpo: SeriesTPOProfile<Time>) => ({
			...tpo,
